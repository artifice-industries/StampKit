...
public final class SKResponse {
    ...
    public static func response(for message: OSCMessage, withDictionary dictionary: [String : Any]? = nil, andCode code: SKResponseStatusCode) -> OSCMessage? {
        switch message.type {
    ...
    // MARK:- Note
    private static func noteCreateResponse(for message: OSCMessage, withDictionary dictionary: [String : Any]? = nil, andCode code: SKResponseStatusCode) -> OSCMessage {
        var noteColour = SKNoteColour.green
        if let dict = dictionary, let colour = dict[SKResponseKeys.colour.rawValue] as? SKNoteColour {
            noteColour = colour
        ...
        var noteText = ""
        if let dict = dictionary, let note = dict[SKResponseKeys.text.rawValue] as? String {
            noteText = note
